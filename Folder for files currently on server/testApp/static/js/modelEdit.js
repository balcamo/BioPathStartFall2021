...
var mouseDown = false; //boolean to determine if mouse is down in canvas. Default false
...
var popUpKey = document.getElementById("key");
popUpKey.addEventListener("click",popUpTheKey);

function popUpTheKey(){
	/*Last edited by Brandon Clark on April 17,2021 in order to add functionality to button to allow html to show up in the popup if the key button is pressed*/
	var popup = document.getElementById("myPopup");
	if (popup.innerHTML == ""){
		popup.innerHTML ="<br><p style=color:green; style><b>Green rectangle = Empty Substrate</b></p>" +
		"<br><p style=color:#FF8C00;style><b>Orange rectangle = Partially Filled Substrate </b></p>"+
		"<br><p style=color:red;style><b>Red rectangle = Filled Substrate </b></p>"+
		"<br><p><b>Arrow = Molecule</b></p>"+
		"<br><p><b>Circles = Enzymes</b></p>"+
		"<br>---------------------------------------------";
...
//el.onwheel = zoom; No longer being used. 4/25/2021 Sean Letts
var displayImages = true; //If true then it will display images instead of the typical dots
var zoomCount; //counts the amoutn of times someone has zoomed in. If they've zoomed in more than 3 times, or zoomed out more than 3, it stops them
//+3 means zoomed in 3 times. -3 means zoomed out 3 times.
var zoomAllowance;
...
/*
Last editted by Sean Letts 2/28/2021
Input: takes in mouse event
Purpose: to allow a user to zoom in/out of the canvas
It restrins how far someone can movee in and out
Outputs: None
Originally Started by Brandon Clark

No longer being used. 4/25/2021 Sean Letts
*/
/*
function zoom(event) {
    ...
    if(scale > 1){
        ...
        if(zoomCount%2 == 0){
            //If someone zoomed in twice, it allows someone to zoom out once
            //2 to one ratio is needed
            zoomAllowance += 1;
        }
    ...
    if(zoomCount < 3 || (zoomOut && zoomCount == 0)){
        //does the zooming if conditions are met.
        context.save();
...
// Calculates the orientation of a dot based on its starting X and Y and it's current X and Y
// Second dot signifies if there is a split in the pathway
...
Last Editted by Sean Letts 2/28/2021
Input: Nothing
Purpose: create zooming functionality
Create mouse reactions when the program is initliazed
These mouse reactions only apply to the canvas.
Output: Nothing
*/
function zoomingFunction(){
    ...
    //plus button element for zooming
    ...
    //minus button element for zooming
    ...
    //mouse functions, can be used for more than just zooming in the future
    ...
    canvas.addEventListener("mousemove",function(evt){
        if(mouseDown){
            //if the mouse is down translate the entire drawing/object in the canvas 
            //direction moved based on position versus the size of the canvas.
            //Center middle will move straight to the left for example.
            var xMove = 0;
            var yMove = 0;
            context.clearRect(0,0,canvas.width,canvas.height);
            ...
            else if(evt.clientY <= 300){
                yMove = -1;
            }
            else{
                yMove = 0;
            }
            /* my failed attempt to do it programatically
            if(evt.clientX >= canvas.width/2 + 300){
                xMove = 1;
            }
            else if(evt.clientX <= canvas.width/2){
                xMove = -1;
            }
            else{
                xMove = 0;
            }
            if(evt.clientY >= canvas.height/2 + 150){
                yMove = 1;
            }
            else if(evt.clientY <= canvas.height/2 - 50){
                yMove = -1;
            }
            else{
                yMove = 0;
